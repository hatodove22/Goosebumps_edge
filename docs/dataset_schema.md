# データセットスキーマ - Goosebumps EdgeAI（Schema v1.1）

更新日: 2026-01-15

本書は、データ取得システム（Collector）が保存する**セッションフォルダ**の構造と、各CSV/JSONの列定義を定める。  
後工程（FFTゲート、学習、組込み）との互換性を最優先し、破壊的変更は避ける。

> v1.1 では **imu.csv** の定義を追加し、実装の挙動（frames.csv の drop_flag、filename が空になるケース）を明文化した。
...
### 2.1 目的
- セッション条件（治具、照明、ソフトウェア版など）を固定し、再現性を担保する。
...
### 3.1 目的
- 受信フレームのタイムスタンプとファイル名の対応を記録する。

### 3.2 列定義
|列|型|説明|
|---|---|---|
|frame_id|int|連番ID（device側）|
...
|drop_flag|int|1なら重複/再送（保存対象外）|

**運用ルール**
- `drop_flag=1` の行は解析から除外する（filenameが空の場合あり）。

---

## 4. imu.csv（任意だが推奨 / v1.2で出力）

### 4.1 目的
- フレーム時点の運動量を記録し、ブレ・動きの大きい区間を除外する補助にする（motion_flag）。
...
### 5.1 目的
- 刺激区間や鳥肌ラベル、照明操作をイベントログとして保存し、後からフレームラベルに展開する。
...
### 6.1 目的
- 露出・ブレ・縞・動きなどの品質指標をフレームごとに記録し、除外や解析に用いる。
...
|blur_laplacian_var|float|ブレ指標|
|banding_score|float|縞スコア（粗い判定）|
